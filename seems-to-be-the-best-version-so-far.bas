...
Private Const DC_SZV_PCT    As Integer = 8   ' Szolg√°ltat√°si sz√≠nvonal 30 mp (%)
Private Const DC_SZV_DB     As Integer = 9   ' Szolg√°ltat√°si sz√≠nvonal 30 mp (db)
Private Const DC_MEGV_PCT   As Integer = 10  ' Megv√°laszol√°si ar√°ny (%)
Private Const DC_VESZT_DB   As Integer = 11  ' Vesztett h√≠v√°s (db)
Private Const DC_VESZT_PCT  As Integer = 12  ' Vesztett h√≠v√°s (%)
...
Private Const LBL_AVG   As String = "¡Åtlag"
...
Private Function ImportOneFile(wbSrc As Workbook, wbDst As Workbook, _
    ...
    ' --- Write header row -------------------------------------------
    wsNew.Cells(1, DC_IDOSZAK).Value = "Idıszak"
    wsNew.Cells(1, DC_C).Value = "Fogadott hÌv·sok"
    wsNew.Cells(1, DC_K).Value = "V·rakoz·s nÈlk¸l kiszolg·lt"
    wsNew.Cells(1, DC_L).Value = "V·rakoz·s ut·n kiszolg·lt"
    wsNew.Cells(1, DC_T).Value = "Munkat·rs ·ltal kiszolg·lt"
    wsNew.Cells(1, DC_AJ).Value = "‹dvˆzlı hangbemond·s"
    wsNew.Cells(1, DC_AU).Value = "HÌv·selvesztÈsek sz·ma"
    wsNew.Cells(1, DC_SZV_PCT).Value = "Szolg·ltat·si szÌnvonal 30 mp (%)"
    wsNew.Cells(1, DC_SZV_DB).Value = "Szolg·ltat·si szÌnvonal 30 mp (db)"
    wsNew.Cells(1, DC_MEGV_PCT).Value = "Megv·laszol·si ar·ny (%)"
    wsNew.Cells(1, DC_VESZT_DB).Value = "Vesztett hÌv·s (db)"
    wsNew.Cells(1, DC_VESZT_PCT).Value = "Vesztett hÌv·s (%)"
...
Private Sub BuildSummary(wbDst As Workbook, sheetNames() As String, sheetCount As Integer)
    ...
    ' Header row
    wsSumm.Cells(1, DC_IDOSZAK).Value = "Idıszak"
    wsSumm.Cells(1, DC_C).Value = "Fogadott hÌv·sok"
    wsSumm.Cells(1, DC_K).Value = "V·rakoz·s nÈlk¸l kiszolg·lt"
    wsSumm.Cells(1, DC_L).Value = "V·rakoz·s ut·n kiszolg·lt"
    wsSumm.Cells(1, DC_T).Value = "Munkat·rs ·ltal kiszolg·lt"
    wsSumm.Cells(1, DC_AJ).Value = "‹dvˆzlı hangbemond·s"
    wsSumm.Cells(1, DC_AU).Value = "HÌv·selvesztÈsek teljes sz·ma"
    wsSumm.Cells(1, DC_SZV_PCT).Value = "Szolg·ltat·si szÌnvonal 30 mp (%)"
    wsSumm.Cells(1, DC_SZV_DB).Value = "Szolg·ltat·si szÌnvonal 30 mp (db)"
    wsSumm.Cells(1, DC_MEGV_PCT).Value = "Megv·laszol·si ar·ny (%)"
    wsSumm.Cells(1, DC_VESZT_DB).Value = "Vesztett hÌv·s (db)"
    wsSumm.Cells(1, DC_VESZT_PCT).Value = "Vesztett hÌv·s (%)"
