...
'   1  = Id≈ëszak          (time interval, source col B)
'   2  = C  - Fogadott h√≠v√°sok
'   3  = K  - V√°rakoz√°s n√©lk√ºl kiszolg√°lt
'   4  = L  - V√°rakoz√°s ut√°n kiszolg√°lt
'   5  = T  - Munkat√°rs √°ltal kiszolg√°lt
'   6  = AJ - √údv√∂zl≈ë hangbemond√°s
'   7  = AU - H√≠v√°selveszt√©sek teljes sz√°ma
'   8  = Szolg√°ltat√°si sz√≠nvonal 30 mp (%) = AA+AC+AE
'   9  = Szolg√°ltat√°si sz√≠nvonal 30 mp (db) = Z+AB+AD
'   10 = Megv√°laszol√°si ar√°ny (%) = T/C
'   11 = Vesztett h√≠v√°s (db) = AU-AJ
'   12 = Vesztett h√≠v√°s (%) = AU/C
' =============================================================
...
Private Const DC_SZV_PCT    As Integer = 8   ' Szolg·ltat·si szÌnvonal 30 mp (%)
Private Const DC_SZV_DB     As Integer = 9   ' Szolg·ltat·si szÌnvonal 30 mp (db)
Private Const DC_MEGV_PCT   As Integer = 10  ' Megv·laszol·si ar·ny (%)
Private Const DC_VESZT_DB   As Integer = 11  ' Vesztett hÌv·s (db)
Private Const DC_VESZT_PCT  As Integer = 12  ' Vesztett hÌv·s (%)
Private Const TOTAL_DST_COLS As Integer = 12

' Output layout
Private Const OUT_HDR_ROWS  As Integer = 3
Private Const OUT_STAT_ROWS As Integer = 4
' Row 1 = title, Row 2 = blank/entity list, Row 3 = column headers
' Stat rows 4-7, data from row 8

' Stat row labels and colours
Private Const LBL_TOTAL As String = "÷sszeg"
Private Const LBL_AVG   As String = "¡tlag"
...
Private Function ImportOneFile(wbSrc As Workbook, wbDst As Workbook, _
    ...
    ' --- Write title (row 1) from source cell A2 -------------------
    Dim titleVal As Variant
    titleVal = wsSrc.Cells(2, 1).Value
    If IsEmpty(titleVal) Or titleVal = "" Then titleVal = StripExtension(sFileName)
    wsNew.Cells(1, DC_IDOSZAK).Value = titleVal
    With wsNew.Cells(1, DC_IDOSZAK).Font
        .Bold = True
        .Size = 14
    End With
    ' Row 2 intentionally left blank

    ' --- Write header row (row 3 = OUT_HDR_ROWS) --------------------
    wsNew.Cells(OUT_HDR_ROWS, DC_IDOSZAK).Value = "Idıszak"
    wsNew.Cells(OUT_HDR_ROWS, DC_C).Value = "Fogadott hÌv·sok"
    wsNew.Cells(OUT_HDR_ROWS, DC_K).Value = "V·rakoz·s nÈlk¸l kiszolg·lt"
    wsNew.Cells(OUT_HDR_ROWS, DC_L).Value = "V·rakoz·s ut·n kiszolg·lt"
    wsNew.Cells(OUT_HDR_ROWS, DC_T).Value = "Munkat·rs ·ltal kiszolg·lt"
    wsNew.Cells(OUT_HDR_ROWS, DC_AJ).Value = "‹dvˆzlı hangbemond·s"
    wsNew.Cells(OUT_HDR_ROWS, DC_AU).Value = "HÌv·svesztÈsek teljes sz·ma"
    wsNew.Cells(OUT_HDR_ROWS, DC_SZV_PCT).Value = "Szolg·ltat·si szÌnvonal 30 mp (%)"
    wsNew.Cells(OUT_HDR_ROWS, DC_SZV_DB).Value = "Szolg·ltat·si szÌnvonal 30 mp (db)"
    wsNew.Cells(OUT_HDR_ROWS, DC_MEGV_PCT).Value = "Megv·laszol·si ar·ny (%)"
    wsNew.Cells(OUT_HDR_ROWS, DC_VESZT_DB).Value = "Vesztett hÌv·s (db)"
    wsNew.Cells(OUT_HDR_ROWS, DC_VESZT_PCT).Value = "Vesztett hÌv·s (%)"
...
Private Sub BuildSummary(wbDst As Workbook, sheetNames() As String, sheetCount As Integer)
    ...
    ' --- Title row (row 1) ---
    wsSumm.Cells(1, DC_IDOSZAK).Value = Chr(214) & "SSZEGZ" & Chr(201) & "S"
    With wsSumm.Cells(1, DC_IDOSZAK).Font
        .Bold = True
        .Size = 14
    End With

    ' --- Entity list (row 2): names read from row 1 of each imported sheet ---
    Dim entityList As String
    entityList = ""
    Dim si As Integer
    For si = 0 To sheetCount - 1
        Dim wsEnt As Worksheet
        Set wsEnt = wbDst.Sheets(sheetNames(si))
        Dim entName As Variant
        entName = wsEnt.Cells(1, DC_IDOSZAK).Value
        If Not IsEmpty(entName) And entName <> "" Then
            If entityList = "" Then
                entityList = CStr(entName)
            Else
                entityList = entityList & ", " & CStr(entName)
            End If
        End If
    Next si
    wsSumm.Cells(2, DC_IDOSZAK).Value = entityList
    With wsSumm.Cells(2, DC_IDOSZAK).Font
        .Bold = True
        .Size = 11
    End With

    ' --- Column headers (row 3 = OUT_HDR_ROWS) ---
    wsSumm.Cells(OUT_HDR_ROWS, DC_IDOSZAK).Value = "Idıszak"
    wsSumm.Cells(OUT_HDR_ROWS, DC_C).Value = "Fogadott hÌv·sok"
    wsSumm.Cells(OUT_HDR_ROWS, DC_K).Value = "V·rakoz·s nÈlk¸l kiszolg·lt"
    wsSumm.Cells(OUT_HDR_ROWS, DC_L).Value = "V·rakoz·s ut·n kiszolg·lt"
    wsSumm.Cells(OUT_HDR_ROWS, DC_T).Value = "Munkat·rs ·ltal kiszolg·lt"
    wsSumm.Cells(OUT_HDR_ROWS, DC_AJ).Value = "‹dvˆzlı hangbemond·s"
    wsSumm.Cells(OUT_HDR_ROWS, DC_AU).Value = "HÌv·svesztÈsek teljes sz·ma"
    wsSumm.Cells(OUT_HDR_ROWS, DC_SZV_PCT).Value = "Szolg·ltat·si szÌnvonal 30 mp (%)"
    wsSumm.Cells(OUT_HDR_ROWS, DC_SZV_DB).Value = "Szolg·ltat·si szÌnvonal 30 mp (db)"
    wsSumm.Cells(OUT_HDR_ROWS, DC_MEGV_PCT).Value = "Megv·laszol·si ar·ny (%)"
    wsSumm.Cells(OUT_HDR_ROWS, DC_VESZT_DB).Value = "Vesztett hÌv·s (db)"
    wsSumm.Cells(OUT_HDR_ROWS, DC_VESZT_PCT).Value = "Vesztett hÌv·s (%)"
    ...
    For i = 0 To n - 1
        ...
        End If
        ' Szolg√°ltat√°si sz√≠nvonal %: sum of AA+AC+AE is not directly available,
